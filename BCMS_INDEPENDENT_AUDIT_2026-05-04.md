...
2. DLQ topology — **PR-A schema done** (`74aff94`); **PR-B1 Schedule shadow done** (`de468b5`); **PR-B2 Booking shadow done** (`4817798`); **PR-B3a Notification shadow done** (`8dc2716`); **PR-B3b-1 Ingest INGEST_NEW shadow done** (`2fea8ed`); **PR-B3b-2 schema (idempotency_key) done** (`49c0daa`, decision `3dfcfdb` sub-option B2); **PR-B3b-2 impl (INGEST_COMPLETED worker+callback shadow) done** (`122a9f1`); **PR-C scope doc done** (`4d85bb1`); **PR-C1 poller bring-up done** (`a25efb6` — non-authoritative); **PR-C2 cut-over runbook done** (henüz commit edilecek); **PR-C2 implementation production soak gate pending** (PR-C1 production deploy + ≥1 saat soak + smoke pre PASS); PR-D (replay) pending. (Madde 7 ile birleşik program; scope doc `c6b1c86`.)
...
5. Live-plan ayrı entity migration — **decision lock done** (`729c74c`, `ops/DECISION-LIVE-PLAN-DATA-MODEL-V1.md` 14 karar + 1 open + 1 deferred). Strangler M5-B1...B6 (+B7 opsiyonel) implementation pending; pre-req production SQL §6 ayrı onay. `channel_id NULL` workflow state olarak kabul edildi (3.2.2 severity revize); asıl mimari iş = live-plan'ın kendi tablosuna taşınması.
6. Schedule-list 2385 satır component refactor
7. Outbox pattern — same V1 program; **PR-B1+B2+B3a+B3b-1+B3b-2 (schema+impl) + PR-C1 poller bring-up done** (`de468b5`, `4817798`, `8dc2716`, `2fea8ed`, `49c0daa`, `122a9f1`, `a25efb6`); **Phase 2 shadow tüm domain'lerde aktif; poller deployed but non-authoritative**; **PR-C2 cut-over implementation production soak gate pending** (cut-over runbook hazır); replay (PR-D) pending.
...
- **[DÜŞÜK] 3.1.21** (yeni 2026-05-06; M5-A investigation sırasında keşfedildi) `schedules.content_id Int?` orphan kolon — schema'da `Content` veya `Asset` modeli yok; `@relation` tanımı yok; FK değil; iş mantığı yok (Zod validation + DB write-through dışında). Yarım kalmış tasarım veya external system reference olabilir. **Madde 5 scope'unda ele alınmadı**; ayrı triage: kolon DROP / rename / canonical asset entity yarat. Asset entity'sinin yokluğu lineage V2 (cross-domain traceability) için de pre-req — bkz `ops/DECISION-LIVE-PLAN-DATA-MODEL-V1.md` §1 ve §3 lock #12.
...
- ~~**[ÖNEMLİ] 3.2.2**~~ → **[BİLGİ] 3.2.2** (severity revize 2026-05-06, kullanıcı domain açıklaması sonrası) — 129/132 schedule `channel_id IS NULL`. **`channel_id NULL` bug değil**, workflow state — kanal yayın akışı kurgu aşamasında atanmamış olabilir; live-plan kanal-merkezli değil; bant içerik kanaldan bağımsız üretilebilir. Live-plan'ın çakışma kontrolü zaten **port-bazlı** (`ingest_plan_item_ports` GIST exclusion). Asıl mimari iş = live-plan'ı kendi tablosuna taşımak (Madde 5; `ops/DECISION-LIVE-PLAN-DATA-MODEL-V1.md`). Eski formülasyon (`channel_id NULL = data quality bug`) yanlıştı.
