...
function isSystemEng(claims: JwtPayload): boolean {
...
export class BookingService {
  ...
  async findAll(request: FastifyRequest, scheduleId?: number, group?: string, page = 1, pageSize = 50) {
    ...
    const currentUserType = isSystemEng(claims) ? 'supervisor' : await fetchUserType(claims.preferred_username);
    const canAssignGroups = isSystemEng(claims) || currentUserType === 'supervisor' ? visibleGroups : [];
