...
> **Son güncelleme**: 2026-05-06 — **Madde 5 live-plan strangler M5-B5 done** (B1 schema → B2 service/API → B3 K15 mapping doc → B4 lookup foundation + metadata DROP → B5 lookup management API). Yeni: `live_plan_entries` (1:1 ile event), 25 lookup tablosu (transmission/* + technical_companies + live_plan_* + fiber_*), `/api/v1/live-plan` + `/api/v1/live-plan/lookups/:type` API, PERMISSIONS.livePlan + livePlanLookups namespace'leri, K15 (JSON YOK; tek `live_plan_technical_details` tablo) + L1-L12 lock'lar. M5-B6 (lookup UI) → M5-B7 (technical_details schema) → M5-B14. Sırada: lookup admin sayfası (frontend ilk PR). Eş zamanlı **Outbox + DLQ V1 (Madde 2+7)**: Phase 2 shadow tüm domain'lerde aktif; PR-C1 poller deployed non-authoritative; PR-C2 cut-over production soak gate pending; PR-D requirements locked. Önceki tur (2026-05-01): RBAC yeniden yapılandırma (Admin tek full-yetki); recording port normalize; OPTA cascade; auth 403 fix.
...
19. **Outbox + DLQ V1 (Madde 2+7, 2026-05-06)**: Tüm domain event'leri `outbox_events` tablosuna **Phase 2 shadow** olarak yazılır (status='published' default; PR-C2 cut-over'da 'pending' olur). 6 event tipi route edilir (`schedule.created/updated`, `booking.created`, `notification.email_requested`, `ingest.job_started/job_completed`). **Idempotency**: eventId UUID v4 her satırda; cross-producer dedup için opsiyonel `idempotency_key VARCHAR(160)` partial unique index (`WHERE idempotency_key IS NOT NULL`) — Madde 4 / PR-A pattern (Prisma 5 partial @@unique sınırlı; UNIQUE migration SQL + test interim helper). Helper: `outbox/outbox.helpers.ts:writeShadowEvent(tx, input)`. **Poller**: `outbox-poller` background service (`OUTBOX_POLLER_ENABLED` env-gated, default false); FOR UPDATE SKIP LOCKED + 2sn polling + 5,10,20,40,80sn backoff (cap 30dk); MAX_ATTEMPTS=5 → dead. Tasarım: `ops/REQUIREMENTS-OUTBOX-DLQ-V1.md`, `ops/REQUIREMENTS-OUTBOX-POLLER-CUTOVER-V1.md`, `ops/REQUIREMENTS-OUTBOX-PR-D-V1.md`; cut-over runbook: `ops/RUNBOOK-OUTBOX-POLLER-CUTOVER.md`; smoke check: `ops/scripts/check-outbox-cutover.mjs`.
20. **Madde 5 live-plan data model — strangler V1 (2026-05-06)**: `schedules` tablosundan ayrılan canonical live-plan domain. M5-B1..B5 done; M5-B6..B14 sıraya. Yeni entity'ler: `live_plan_entries` (event-level 1:1; M5-B1), `live_plan_technical_details` (~80 kolon prefix'li; M5-B7), `live_plan_transmission_segments` (1:N + feed_role; M5-B8), 25 lookup tablo (transmission/* + technical_companies + live_plan_* + fiber_audio/video_formats; M5-B4). API yüzeyleri: `/api/v1/live-plan` (CRUD + If-Match optimistic lock; M5-B2) + `/api/v1/live-plan/lookups/:type` (generic CRUD + 25-tip whitelist + RBAC; M5-B5). **K15 prensibi**: JSON canonical YOK; teknik detaylar structured DB kolonu/lookup FK; legacy migration yok (yeni model boş başlar). **L1-L12 lock'lar**: tek LookupService + adapter; L5 DELETE soft + restore PATCH deletedAt:null; L8 includeDeleted write-yetkili kullanıcıya; L9 ORDER active DESC + sort_order ASC; L10 PATCH .strict() + deletedAt yalnız null; L11 polymorphic type immutable; L12 lookup'ta optimistic locking yok V1. PERMISSIONS namespace: `livePlan` (Schedule.write/delete clone) + `livePlanLookups` (read all-auth, write/delete SystemEng + Admin). Detay: `ops/DECISION-LIVE-PLAN-DATA-MODEL-V1.md` + `ops/REQUIREMENTS-LIVE-PLAN-TECHNICAL-FIELDS-V1.md`. **`channel_id NULL` bug değil**, workflow state.
