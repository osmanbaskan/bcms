...
> **Son güncelleme**: 2026-05-06 — **Outbox + DLQ V1 program (Madde 2+7)** + **Madde 5 live-plan data model decision lock**. Outbox shadow Phase 2 tüm domain'lerde aktif (Schedule + Booking + Notification + Ingest); idempotency_key partial unique (cross-producer dedup, sub-option B2); poller PR-C1 deployed but non-authoritative (`OUTBOX_POLLER_ENABLED` env-gated); PR-C2 cut-over production soak gate pending; PR-D requirements locked (replay/retention/dedup/cleanup). Madde 5: live-plan ayrı entity stranger plan (M5-A decision doc 14 locked + 1 closed + 1 deferred); local dev DB inventory yapıldı (§4.1 booking 0 satır → closed); M5-B1 schema PR teknik olarak unblocked. Önceki tur (2026-05-01): RBAC yeniden yapılandırma (SystemEng "ops super-grubu" davranışı kaldırıldı, **Admin tek full-yetki grubu**); recording port normalize; OPTA cascade; auth interceptor 403 fix; postgres_backup sidecar.
...
20. **Madde 5 live-plan data model — strangler V1 (2026-05-06, decision lock)**: `schedules` tablosu iki ayrı domain entity'yi (yayın akışı + operasyon üst planı) tek tabloda taşıyor. Karar: live-plan kendi tablosuna (`live_plan_entries`) taşınır; schedule = broadcast slot + reporting context. **`channel_id NULL` bug değil**, workflow state. Strangler: M5-A (decision doc) → M5-B1 (foundation) → M5-B2 (service split) → M5-B3 (ingest live FK) → M5-B4 (ingest studio FK + XOR CHECK) → M5-B5 (cleanup) → M5-B6 (UI ayrımı; Madde 6 ayrı PR) → M5-B7 (sourceType cleanup, deferred). Decision: `ops/DECISION-LIVE-PLAN-DATA-MODEL-V1.md`. Live-plan port taşımaz (port `ingest_plan_items` authoritative). Asset entity yok; `schedule.content_id` orphan kolon (ayrı triage; lineage V2 pre-req).
