...
ingest.reportIssue:      [] (tüm authenticated — gelecekte mail entegrasyonu)
...
Canlı Yayın Plan Listesi ekranında her satırda çoğaltma (+) ve silme (çöp kutusu) arasında **Sorun Bildir** (⚠ report_problem) ikonu bulunur.

- Görünürlük: `canReportIssue` computed signal — `['SystemEng', 'Tekyon', 'Transmisyon']`
- Tıklandığında `ReportIssueDialogComponent` açılır (inline dialog, `schedule-list.component.ts` içinde tanımlı)
- Dialog: readonly içerik bilgileri (başlık, tarih, saat, kanal) + serbest metin textarea (max 2000 karakter)
- Kayıt: `POST /api/v1/incidents/report` → `incidents` tablosuna `eventType='SCHEDULE_ISSUE'`, `severity='ERROR'`
- Metadata: `{ title, startTime, endTime, channel, reportedBy }` — `reportedBy` JWT `preferred_username`'dan alınır
- İleride mail entegrasyonu planlanmaktadır (şu an kayıt sadece DB'ye yazılır)

API endpoint: `apps/api/src/modules/incidents/incident.routes.ts` → `POST /report`
Frontend: `apps/web/src/app/features/schedules/schedule-list/schedule-list.component.ts` → `ReportIssueDialogComponent`
...
apps/web/src/app/features/schedules/schedule-list/schedule-list.component.ts → ReportIssueDialogComponent + canReportIssue
apps/api/src/modules/incidents/incident.routes.ts → POST /report (Sorun Bildir endpoint)
