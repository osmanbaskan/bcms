...
> Son güncelleme: 2026-05-11 — **Y5-8 Schedule.channelId Prisma + kod DROP done** (`70a7150`); DB migration `20260511120000_drop_legacy_schedule_channel_id` runtime apply bekliyor. **Madde 5 strangler M5-B1..B6 + M5-B7..B8 backend + M5-B10a done**, M5-B10b preflight doc (UI APPROVAL bekliyor). **SCHED-B5a Block 2 apply done** (`52db5a9`, 2026-05-10): legacy `usage_scope` + `deleted_at` + `schedules_no_channel_time_overlap` GiST DROP + BXF tamamen kaldırıldı. **MCR sekmesi + playout backend kaldırıldı** (`0e10e62`, runtime apply done). **Timezone Lock + Helper** (`Europe/Istanbul`, `cc99841` + `5e5cf2c` + `8658e32`) done; reporting TZ drift B5b'ye kaldı. **Smoke script** canonical hizalama (`f058c73`, `/health` minimal). Preflight doc'ları: `ops/REQUIREMENTS-STATUS-MODEL-RETHINK-V1.md` (ON_AIR rethink), `ops/REQUIREMENTS-REPORTING-CANONICALIZATION-B5B.md` (B5b BLOCKED), `ops/REQUIREMENTS-LIVE-PLAN-TECHNICAL-FORM-M5-B10B.md` (UI APPROVAL bekliyor). **Outbox + DLQ V1**: Phase 2 shadow + PR-C1 poller deployed non-authoritative; PR-C2/PR-D production soak gate pending.
...
> **B5b'ye kalan legacy schedule kolonları**: `schedules.metadata`, `start_time`, `end_time` (reporting `/schedules/reporting` bağımlı; UI freeze altında BLOCKED — bkz. `ops/REQUIREMENTS-REPORTING-CANONICALIZATION-B5B.md`). `channel_id` + `Schedule.channel` relation Prisma model'inden Y5-8 (`70a7150`, 2026-05-11) ile kaldırıldı; DB migration `20260511120000_drop_legacy_schedule_channel_id` runtime apply'ı bekliyor. Yeni kod bu alanlara bağlanmasın.
