...
| ✅ Locked (broadcast flow split) | K16 / K1-K32 | §3.5 — Schedule/Yayın Planlama alanları; event_key; 3 kanal slotu; JSON cleanup hedefi |
...
| 8 | **Eski `schedules.usageScope='live-plan'` satırları cleanup, migration shim yok.** | K16/K1-K32 kararıyla eski live-plan schedule kayıtları migrate edilmez; destructive cleanup kabul edildi. |
| 9 | **`usageScope` kolonu kalır kararı superseded.** | 2026-05-07 K16/K2 kararıyla `schedules.usage_scope` hedefte DROP edilir. Bu satır tarihsel M5-A kararının revizyonunu kayda geçirir. |
| 10 | **Schedule kaldırılmaz; Yayın Planlama domain'i olarak daraltılır.** | Yayın planlama bağımsız domain; kanal/ticari/logo/format akışını tutar; live-plan operasyon/teknik detay domain'i ile aynı yere düşmez. |
...
- Eski `schedules.usageScope='live-plan'` audit history `subject="Schedule"` olarak **historical kalır**; destructive cleanup sırasında audit history migrate edilmez.
...
### 3.5 K16 — Schedule/Yayın Planlama domain split + channel propagation (2026-05-07)

Bu bölüm kullanıcı domain ownership düzeltmesiyle lock'landı: **Canlı Yayın Plan, Stüdyo Planı, Schedule/Yayın Planlama ve Ingest aynı domain değildir**. Canlı Yayın Plan altındaki OPTA + manuel event verileri `live_plan_*` DB domain'inde yaşar. Schedule/Yayın Planlama ise "hangi içerik hangi kanalda/saatte yayınlanacak?" bilgisini tutar. Ingest, live-plan/studio-plan'dan gelen **isim + saat** bilgisini kendi ingest DB kayıtlarında takip eder; ana event kaynağı değildir. Schedule/Yayın Planlama'dan Ingest'e veri akışı yoktur.

Detaylı requirements dosyası: **`ops/REQUIREMENTS-SCHEDULE-BROADCAST-FLOW-V1.md`**.

#### K16.1 — UI / navigation

- Ayrı **"Live-Plan (yeni)"** menü etiketi kaldırılır.
- Sol menü:
  - **Canlı Yayın Plan** → `/live-plan` (`live_plan_*` structured DB ekranı).
  - **Yayın Planlama** → `/schedules` (Schedule domain ekranı).
- Lookup yönetim ekranının kullanıcı etiketi **"Lookup Yönetimi"** olur; route şimdilik `/admin/live-plan-lookups` kalır.

#### K16.2 — JSON/metadata cleanup hedefi (K1-K4, K22-K24)

- Eski live-plan JSON dataları migrate edilmez; silinebilir.
- `schedules.usage_scope='live-plan'` kayıtları cleanup scope'undadır; migration ile DELETE edilir.
- `schedules.metadata` canonical kaynak olmayacak; hedef komple DROP.
- `schedules.usage_scope`, `schedules.start_time`, `schedules.end_time`, `schedules.channel_id` hedefte DROP edilir.
- Eski live-plan schedule satırları silinirken bağlı `bookings` / `incidents` / `timeline_events` kayıtları cascade ile silinebilir. **Veri kaybı bilinçli kabul.** Test/production ayrımı garanti edilmez.
- Eski `ingest_plan_items.sourceType='live-plan'` bağlantılı kayıtlar temizlenir; doğru bağlantı M5-B11'de `live_plan_entry_id` FK ile yeniden kurulur.
- DROP migration'dan önce grep + DB inventory zorunludur. Metadata kullanan gerçek path varsa önce structured kolona taşınır veya silinir.

#### K16.3 — Event identity (K9, K18-K21)

- Schedule ile Live-Plan ortak **`event_key`** ile bağlanır.
- Format:
  - OPTA: `opta:<optaMatchId>`
  - Lokal match: `match:<matchId>`
  - Manuel: `manual:<generatedUuid>`
- `schedules.event_key` UNIQUE olur: Schedule/Yayın Planlama tarafında aynı event için tek satır.
- `live_plan_entries.event_key` non-unique olur: aynı event için birden çok live-plan entry olabilir.
- Ingest tarafında aynı event için birden çok ingest item olabilir.

#### K16.4 — Source type + content selection (K6-K8, K18)

- `live_plan_entries.source_type = 'OPTA' | 'MANUAL'`.
- Schedule içerik seçimi `live_plan_entries` üzerinden yapılır:
  - "OPTA'dan Seç" → `source_type='OPTA'`
  - "Manuel İçerik" → `source_type='MANUAL'`
- Manuel live-plan entry hem Canlı Yayın Plan sekmesinden hem Schedule/Yayın Planlama "Manuel İçerik Ekle" akışından oluşturulabilir; her durumda kayıt `live_plan_entries` tablosuna yazılır.
- Schedule kendi manuel içerik tablosu oluşturmaz.
- `team_1_name` / `team_2_name` Schedule ekranında yazılmaz; seçilen `live_plan_entries` içeriğinden kopyalanır.

#### K16.5 — Schedule/Yayın Planlama alanları (K10, K17, K30-K32)

Schedule ekranında hedef kolon seti:
...
```text
TARİH | TAKIM 1 | TAKIM 2 | SAAT | KANAL 1 | KANAL 2 | KANAL 3 | TİCARİ | LOGOLAR | FORMAT
```

Alan kuralları:

- Required: `event_key`, `schedule_date`, `schedule_time`.
- Opsiyonel: kanal slotları, `commercial_option_id`, `logo_option_id`, `format_option_id`.
- `selected_live_plan_entry_id` seçilen entry'yi izlemek için tutulur; kanal propagation bu FK üzerinden değil `event_key` üzerinden yapılır.
- `TİCARİ`, `LOGOLAR`, `FORMAT` select box'tır; lookup FK olarak tutulur.
- `TARİH` + `SAAT` tek schedule zamanı temsil eder. Eski `start_time` / `end_time` schedule için canonical değildir; transmisyon süreleri `live_plan_transmission_segments`, ingest saatleri ingest domain'indedir.

#### K16.6 — Kanal slotları ve propagation (K5, K11-K12)

- Çoklu kanal child table değil, sabit slot modeli:
  - `channel_1_id`
  - `channel_2_id`
  - `channel_3_id`
- Aynı satırda duplicate kanal yasak: `channel_1_id`, `channel_2_id`, `channel_3_id` birbirini tekrar edemez.
- Schedule kanal slotları aynı `event_key`'e sahip tüm `live_plan_entries` kayıtlarına birebir yansır:
...
- Schedule create/update transaction içinde kanal propagation service-level yapılır.
- Yeni live-plan entry oluşturulduğunda aynı `event_key` için schedule varsa kanal slotları otomatik kopyalanır.

#### K16.7 — Schedule lookup tabloları (K13-K16)

Yeni lookup tabloları:
...
Bu lookup'lar mevcut lookup management UI içinde yeni **"Yayın Planlama"** grubu olarak yönetilir. Menü adı **"Lookup Yönetimi"** olur; route şimdilik `/admin/live-plan-lookups` kalır.

#### K16.8 — Domain akışı (K25-K29)

- Canlı Yayın Plan → Ingest: sadece **isim + saat**.
- Stüdyo Planı → Ingest: sadece **isim + saat**.
- Schedule/Yayın Planlama → Ingest: **akış yok**.
- Canlı Yayın Plan teknik detayları `live_plan_technical_details` içine yazılır; `schedules.metadata` veya başka JSON canonical kaynak değildir.
- Transmisyon süreleri `live_plan_transmission_segments` içinde kalır; Schedule/Yayın Planlama transmisyon segmenti tutmaz.

#### K16.9 — Eski kolon cleanup prensibi

- Eski `schedules.channel_id` kaldırılacak; yeni `channel_1_id/channel_2_id/channel_3_id` canonical olur.
- `schedules.metadata`, `schedules.usage_scope`, `schedules.start_time`, `schedules.end_time` hedefte kaldırılır.
- Gereksiz eski `schedules` kolonları drop edilecek; destructive migration inventory + usage grep sonrasında, schema/backend/frontend rewiring tamamlandıktan sonra yapılır.
- Yayın Planlama için gereken kolonlar korunur.
...
- Schedule-list M5-B10a'da dokunulmadı. K16/K1-K32 sonrası `/schedules` için ayrı Yayın Planlama rewiring'i SCHED-B4 kapsamına taşındı.
- Test: API client/service unit + segment dialog component smoke.

**M5-B10b — 76 alan technical-details (sonra)**
...
- Y2 REVIZE eski "disable + redirect" yaklaşımı **superseded**. K16/K1-K32 kararıyla `/schedules` artık **Yayın Planlama** domain'i olarak yeniden bağlanır; live-plan teknik detay write path'i sadece `/live-plan` üzerinden çalışır.
- Test: form section component smoke (her mat-tab) + navigation/schedule domain regresyon.
...
**Eski path / koruma (Y2 FINAL, Y8 superseded):**
- **Y2 FINAL (2026-05-07 K16/K1-K32):** Eski Schedule "Edit / Teknik Detay" write path'i Canlı Yayın Plan bağlamında kapatılır; `/schedules` kullanıcı etiketi **Yayın Planlama** olur.
- `usage_scope='live-plan'` satırları destructive cleanup scope'undadır; disable/redirect ara çözümü artık canonical plan değildir.
- Y8 "schedule-list refactor yok" kararı K16 ile daraltıldı: schedule-list büyük refactor değil, fakat Yayın Planlama domain alanlarına surgical rewiring yapılır.
...
- `DELETE FROM schedules WHERE usage_scope='live-plan'` (destructive cleanup; veri kaybı bilinçli kabul).
- Bağlı `bookings` / `incidents` / `timeline_events` kayıtları cascade ile silinebilir.
- Eski `ingest_plan_items.sourceType='live-plan'` bağlantılı kayıtlar temizlenir.
- `schedules.metadata`, `schedules.usage_scope`, `schedules.start_time`, `schedules.end_time`, `schedules.channel_id` hedefte DROP edilir.
- `metadata.liveDetails`, `metadata.transStart`, `metadata.transEnd` artık yazılmaz; historical data UI'da görünmez.
...
Mevcut: `usageScope` CHECK constraint var, integration test mevcut. K16/K2 kararıyla `usage_scope` hedefte DROP edilir; bu finding cleanup migration kapsamına taşınır.
