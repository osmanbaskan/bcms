...
### ✅ K2.4 — GERÇEK execute (DC00042608) — BAŞARILI
**Durum:** TAMAMLANDI (kullanıcı onaylı, gerçek IPWS submit)

`avid-restore-smoke.ts <DC00042608-mobid> --execute` çalıştırıldı:
- ✅ `SubmitJobUsingProfile` → Avid kabul etti, gerçek **JobURI** döndü:
  `interplay://BSVMWG/DMS?jobid=1780247955990.1`
- ✅ `GetJobStatus` defalarca sorgulandı → tutarlı **`running`** (~5+ dk; DIVA restore
  süresi, kodla ilgisiz). `mapJobStatus` doğru çözüyor.
- Submit + poll mekaniği uçtan uca canlı kanıtlandı. Production'da restore worker
  5sn poll ile job'u takip edip Completed→DONE yapacak (worker hazır, dokunulmadı).

**✅ ONLINE TEYİDİ — TAM UÇTAN UCA KANIT:** ~3 dk sonra JobStatus **`done`** oldu;
DC00042608 K1 search'te **`offline → online`** döndü (Media Status ham XML=online,
AvidAsset.online=true). Aynı mobid. Yani: offline asset → SubmitJobUsingProfile →
DIVA→Interplay restore → Completed → asset Interplay'de online. **K2 restore gerçek
Avid'de baştan sona çalıştı.**

| Zaman | Media Status | online |
|---|---|---|
| Restore öncesi (ilk K1 smoke) | offline | false |
| Restore execute → done → sonrası | **online** | **true** |

**Notlar (dürüstlük):** poll script'inde bazı turlar sleep'i atladı (sonuç yine doğru);
